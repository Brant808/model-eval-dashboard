# ROWS — Phase 2 Metric Registry (gate-resolved 2026-08-01)
...
| 2 | Epoch Capabilities Index (`epoch-eci`) | S10 / I +caveat | 5/5 | A PARTIALLY-OVERLAPPING second read of the frontier order (ECI ingests GDPval/METR/Vals/ARC — inputs this page also shows — plus vendor-TR and unsourced rows): agreement is weak corroboration, DISAGREEMENT is the strong signal. Overlap named in the row brief. |
...
| 4 | AA Agentic Index (`aa-agentic-index`) | S1 / I +Gemini-grader caveat | 5/5 (fixture-verified via the S1 collector — all five carry agenticIndex; Fable 52.81) | Who leads specifically on agentic work (GDPval + τ³-Banking composite) as opposed to benchmarks-at-large? Declared overlap with row 3 in both briefs; carries the Gemini-grader caveat because GDPval is ~half its weight. |
...
| 7 | SWE-rebench fresh-issue resolve rate (`swe-rebench`) | S20 / I +per-window contamination flags | 4/5 | Fresh-issue coding tracked window-over-window against ITSELF — never compared numerically against SWE-bench Pro or Verified (three separated scales, linter-enforced). Decontamination is WINDOW-RELATIVE, not absolute: the board itself flags models released after window start as "Potential contamination" (currently Fable 5, Opus 5, Sol; DS V4 Pro is the only clean covered cell) — the collector derives that flag mechanically (release.date > window_start) and every affected cell must carry it. |
| 8 | SWE-bench Pro vendor claims (`swe-bench-pro`) | S13+S14 / V | 4/5 | What are labs CLAIMING on their own coding harnesses — and how big is the claim-vs-STANDARDIZED gap (S9 runs ~20 pts colder)? Quarantine is a BINDING Phase 3/6 obligation, not an accomplished fact: whole-row vendor-claims treatment (tinted "claimed" sub-band + row label), warn-class self-report flags on every cell (linter-enforced via INTEGRITY_MARKERS), chips impossible (all-V). |
...
| 13 | Disclosure watch (`disclosure-watch`) | S21 / I (curated) | 5/5 | Which lab currently owes the field an explanation — the page's disclosure-integrity differentiator. Sourced to S21 (curated synthesis; each open item's flag names its primary source); updated only by governed edit or the schema-locked judgment layer. |
...
| 20 | Vals professional index (`vals-index`) | S11 / I +VC-funding caveat | 5/5 (gate-corrected: DeepSeek V4 Pro IS on the live board — 55.6, rank 19/41) | Does frontier capability convert into completed professional work — with the strict-vs-weighted gap as the live reliability-under-repetition badge (Finance Agent v2 gaps run 10.6–12.7 pts across the five)? |
...
- **HOLD** FAR.AI Security Leaderboard — novel jailbreak-resistance signal (Fable 0 universal jailbreaks vs Grok 448 under 1,500 attacks) but no machine-readable endpoint (hash-named JS bundle) and today's columns would read 0 / — / 0 / — / — (differentiates nothing in-matrix). Trigger (gate-corrected): a machine-readable endpoint exists AND (coverage ≥3 columns OR the in-matrix spread is differentiating). Collectability is the hard gate — an uncollectable row violates zero-input. Its launch feeds disclosure-watch/tape meanwhile.
...
## Honest redundancy accounting (Phase 2 gate)

The 20 rows are NOT 20 independent signals — roughly 15–16 are, and the rest is
deliberate, named redundancy: rows 3+4 overlap by construction (Elo inside the
composite); rows 14+15 are two views of one evaluation; row 17 is arithmetic on
rows 1+16; row 2 partially re-ingests rows 3/4/10/11/20; row 12's withholding
signal also appears in row 13. Cross-checks and calibrated splits earn their
slots as redundancy, not as new information. Concentration: 7 rows ride S1
(1, 3, 4, 14, 15, 16, 17) plus S1-relayed cells in 18 — an accepted
methodology-hostage exposure, mitigated by rows 2/5/7/9/20 being non-AA.
Cut order if the cap ever binds: row 12 → fold row 3 into row 4's brief →
row 17 (its value is reproducible as a quick-look stat).

Cell-definition pins (gate): row 9 = LiveBench GLOBAL AVERAGE
(`livebench-<release>` set); row 20 = the Vals Index WEIGHTED composite with
the strict-vs-weighted gap as a badge. Window/release-stamped sets (rows 7, 9)
reset comparability on rotation: the pipeline pre-classifies rotation-day
changes as "methodology change, not movement" and suppresses cross-set delta
chips. All coverage fractions here are denominated against the CURRENT column
set and restate automatically when Phase 3's catalog changes the denominator.
